...
//            echo "</ul>";
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($_FILES['image']['name']) {
                ...
                $myname = $imageName . $ext; //File name with extension for filesystem
                move_uploaded_file($_FILES['image']['tmp_name'], $save_path . $myname);

                //Insert link to image in database
                $sql = "INSERT INTO PHOTO(Title, Author, Date, Event, EventType, Link)
                    VALUES ('{$imageName}', '{$author}', '{$date}', '{$event}', '', 'nbproject/DatabaseImages/{$myname}')";
